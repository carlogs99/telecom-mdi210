...
public class Descent extends Observable implements Runnable {
	...
	private int delay = 2000; /*
								 * serves to slow the descent for graphical display; delay in milliseconds
								 * between two steps of the descent
								 */
	 ...
	 *           minimum of a convex function of two variables on a domain of the
	 *           plan limited by lines.
	 */
	...
	/**
	 * METHOD TO IMPLEMENT Search the next direction to follow when the current
	 * point P is in the strict interior of the domain. If the descent is not
	 * finished, put the next direction in the attribute this.direction (it is
	 * mandatory to use this attribute). <br>
	...
	private void directionToFollowIfPInside() {
		// Check if method is finished
		if (this.pb.gradientf(this.P).norm() < this.threshold) {
			this.finished = true;
			// Else update direction
		} else {
			// Direction of maximum descent is given by negative of gradient
	...
	/**
	 * METHOD TO IMPLEMENT Search the next direction to follow when the current
	 * point P is on an edge of the domain but not in a corner. If the descent is
	 * not finished, put the next direction in the attribute this.direction (it is
	 * mandatory to use this attribute). <br>
	 * According to the direction of the gradient of f and if the descent is not
	 * completed, the next direction may be towards the strict interior of the
	 * domain or along the edge in one direction or the other. <br>
	 * WARNING: if the descent method is finished, the method must set to true the
	 * attribute this.finished.
	 * 
	 * @see Couple#norm()
	...
	private void directionToFollowIfPOnEdge(Constraint c) {
		...
		// If not finished, we need to check if next direction is along edge
		// n is the vector perpendicular to constraint
		Couple n = c.getGradient();
		if (n.scalarProduct(pb.gradientf(this.P).mult(-1)) >= 0) {
			// Direction is towards interior
			this.direction = pb.gradientf(this.P).mult(-1);
		}
		// u is the vector in the direction of the constraint
		Couple u = c.getUnitaryEdgeVector();
		if (u.scalarProduct(pb.gradientf(this.P).mult(-1)) == 0) {
			// Direction is perpendicular to constraint, towards exterior
			this.finished = true;
		} else if (u.scalarProduct(pb.gradientf(this.P).mult(-1)) > 0) {
			this.direction = u;
		} else {
			this.direction = u.mult(-1);
		}
	}

	/**
	 * METHOD TO IMPLEMENT Search the next direction to follow when the current
	 * point P is in a corner of the domain. If the descent is not finished, put the
	 * next direction in the attribute this.direction (it is mandatory to use this
	 * attribute). <br>
	 * WARNING: if the descent method is finished, the method must set to true the
	 * attribute this.finished. <br>
	 * See the subject on
	 * http://www.infres.enst.fr/~hudry/optim/descentEnglish/index.html for
	 * explanations. The method decompose being static, it is called with
	 * Couple.decompose...
	 * 
	 * @see Couple#norm()
	 * @see Couple#scalarProduct(Couple)
	...
	private void directionToFollowIfPOnCorner(Constraint[] corner) {
		...
		Couple u1 = corner[0].getUnitaryEdgeVector();
		Couple u2 = corner[1].getUnitaryEdgeVector();
		Couple n1 = corner[0].getGradient();
		Couple n2 = corner[1].getGradient();
		Couple negGrad = pb.gradientf(this.P).mult(-1);
		Couple grad = pb.gradientf(this.P);

		// First case
		if (u1.scalarProduct(negGrad) > 0 && u2.scalarProduct(negGrad) > 0) {
			this.direction = negGrad;
		}

		// Second case (KKT stopping condition)
		if (n1.scalarProduct(negGrad) < 0 || n2.scalarProduct(negGrad) < 0) {
			this.finished = true;
		}

		// Third case
		if (u1.scalarProduct(grad) >= u2.scalarProduct(grad)) {
			this.direction = u2;
		} else {
			this.direction = u1;
		}
	...
	/**
	 * METHOD TO IMPLEMENT We consider a half-line parameterized by t ->
	 * startingPoint + t.dir (t> = 0); we set phi(t) = f(startingPoint + t.dir); we
	 * suppose that we have phi'(0) < 0; we look for a point P = startingPoint +
	 * t.dir, t> 0, with phi'(t) > 0. Recall that, by hypothesis, the function f is
	 * convex, which means that phi' is increasing. You can first test t = 1 and
	 * then, if necessary, double the value of t successively. The derivative of t
	 * -> f(startingPoint + t.dir) can be written : pb.phiDerivative(startingPoint,
	 * dir, t); see Pb#phiDerivative (Couple, Couple, double)
	 * 
	 ...
	 *         method returns -1; otherwise the method returns a value of t with
	 *         phi'(t)> 0.
	 */
	public double searchSecondPoint(Couple startingPoint, Couple dir) {
		int t = 1;
		while (t < Double.MAX_VALUE / 2) {
			if (pb.phiDerivative(startingPoint, dir, t) > 0) {
				return t;
			}
			// startingPoint.add(dir.mult(t));
			t = t * 2;
		}
		return -1;
	}
	 ...
	 * METHOD TO IMPLEMENT We consider a half-line parameterized by t ->
	 * startingPoint + t.dir (t> = 0); We set phi(t) = f (startingPoint + t.dir); we
	 * suppose that we have phi'(0) <0 and phi'(t1)> 0; We look for a point P =
	 * startingPoint + t * dir between startingPoint and startingPoint + t1 * dir
	 * with phi'(t) = 0. For this, we proceed by dichotomy. One can use the static
	 * method isNull of this class to test if a double value is null or not. The
	 * derivative of t -> f(startingPoint + t.dir) can be written :
	...
	public Couple dichotomy(Couple startingPoint, Couple dir, double t1) {
		...
		double b = t1;
		double m = (b - a) / 2;
		double phiPrime = pb.phiDerivative(startingPoint, dir, m);
		while (!isNull(phiPrime)) {
			if (phiPrime < 0) {
				a = m;
			} else {
				b = m;
			...
			m = (b - a) / 2;
			phiPrime = pb.phiDerivative(startingPoint, dir, m);
		}
		return startingPoint.add(dir.mult(m));
	}

	/**
	 * METHOD TO IMPLEMENT Check that this is a minimum using the condition of
	 ...
	 * @see Constraint#getGradient()
	 * @see Pb#gradientf(Couple)
	 * @return null if the condition of Karush, Kuhn et Tucker is not satisfied <br>
	 *         otherwise <br>
	 *         - the couple of Lagrange multipliers if P is in a corner <br>
	...
	public Couple KarushKuhnTucker(Couple P) {
		...
		if (domain.isCorner(P) != null) {
			Constraint[] c = domain.isCorner(P);
			lambda = Couple.decompose(pb.gradientf(P).mult(-1), c[0].getGradient(), c[1].getGradient());
			if (lambda.x >= 0 && lambda.y >= 0)
				return lambda;
			else
				return null;
		} else {
			if (domain.isOnEdge(P) != null) {
				Constraint c = domain.isOnEdge(P);
				double ratio1, ratio2 = 0;

				if (c.getGradient().x != 0 && c.getGradient().y != 0) {
					ratio1 = pb.gradientf(P).mult(-1).x / c.getGradient().x;
					ratio2 = pb.gradientf(P).mult(-1).y / c.getGradient().y;
					System.out.println(ratio1 - ratio2);
					if (ratio1 - ratio2 < threshold && ratio1 - ratio2 > -threshold && ratio1 >= 0)
						lambda = new Couple(ratio1, 0);
				} else {
					if (c.getGradient().x != 0) // the y of del(g) is always zero, then
					{
						if (pb.gradientf(P).mult(-1).y == 0) {
							ratio1 = pb.gradientf(P).mult(-1).x / c.getGradient().x;
							if (ratio1 >= 0)
								lambda = new Couple(ratio1, 0);
						}
					} else {
						if (pb.gradientf(P).mult(-1).x == 0) {
							ratio2 = pb.gradientf(P).mult(-1).y / c.getGradient().y;
							if (ratio2 >= 0)
								lambda = new Couple(ratio2, 0);
						}
					}
				}
			} else {
				if (pb.gradientf(P).norm() < threshold) // check the case where we are at the minimum without
														// constraints
	...
	/**
	 * Look for the next direction to follow. The method considers the cases where:
	 * P is inside the domain, P is on an edge, P is on a corner <br>
	 * If the descent is finished, the method sets this.finished to true. </font>
	 */
	 ...
	 * 
	 * @return this.P is the current point (which can be inside domain, on a edge of
	 *         the domain or on a corner) and this.direction is the direction to
	 *         follow; the method returns <br>
	 *         - the next current point (which can be inside the domain, on an edge,
	 *         on a corner) <br>
	 *         - null if it showed that the function f does not reach a minimum on
	 *         the considered domain.
	 */
	public Couple nextP() {
		double t1;
		/*
		 * Explanations for the following instruction. We consider a half-line
		 * parameterezd by t -> this.P + t * this.direction; this half-line can start
		 * from inside the domain, or from an edge, or from a corner and may skirt an
		 * edge of the domain. If the half-line is whole in the domain which is only
		 * possible if the domain is not bounded, the statement returns -1. Otherwise,
		 * it returns a value of t > 0 for which this.P + t * this.direction belongs to
		 * an edge of the domain.
		 */
	 ...
	 *                pursued if the parameter is false.
	 */
	...
	/**
	 * After the descent is completed, it permits to know if the problem has reached
	 * its minimum or not.
	 * 
