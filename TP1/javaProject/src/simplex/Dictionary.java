...
public class Dictionary extends Observable {
	...
	public boolean isFeasible() {
		for(int j = 1 ; j <= this.nbBasicVar ; j++) {
			if(this.D[j][0] < 0) {
				return false;
			}
		}
	...
	public int searchIndexLeavingVariable(int jE)  {
		...
		// Simplex.output.println("\nThe entering variable: x" + this.arrayNonBasicVar[jE]);
		// Simplex.output.println("Method searchIndexLeavingVariable to write");
		// incomplete = true;
		double minConstraint = Double.MAX_VALUE;
		double constraint_i;
		int indexLeavingVar = 0;
		...
		for(int i = 1; i <= this.nbBasicVar ; i++) {
			constraint_i = -this.D[i][0]/this.D[i][jE];
			if(constraint_i < minConstraint && this.D[i][jE] < 0) { 
				minConstraint = constraint_i;
				indexLeavingVar = i;
			}
		}
		return indexLeavingVar; 
	}
	...
	public void pivote(int iS, int jE) {
		// Modify the array containing the indexes of the variables
		int aux = this.arrayBasicVar[iS]; 
		this.arrayBasicVar[iS] = this.arrayNonBasicVar[jE]; // Set to variable entering basic set
		this.arrayNonBasicVar[jE] = aux; // Set to variable leaving basic set
		
		// Modify the equation for the variable leaving the basic set
		// For every coefficient except the one of the variable exiting the basic set:
		for(int j = 0; j <= this.nbNonBasicVar; j++) { 
			if(j != jE) { 
				this.D[iS][j] = -this.D[iS][j] / this.D[iS][jE]; 
			}
		}
		// Modify the coefficient of the variable that exited the basic set:
		this.D[iS][jE] = 1/this.D[iS][jE];
		
		// Go over the rows except the one we already modified:
		for(int i = 0 ; i <= this.nbBasicVar ; i++) {
			if(i != iS) {
				// Go over the columns except the one corresponding to entering variable:
				for(int j = 0 ; j <= this.nbNonBasicVar ; j++) {
					if(j != jE) {
						this.D[i][j] = this.D[i][j] + this.D[i][jE] * this.D[iS][j];
					}
				}
				this.D[i][jE] = this.D[i][jE] * this.D[iS][jE];
			}
		}
	}
	...
	public int searchIndexEnteringVariableGreatestCoeff() {	
		...
		int indexMaxCoeff = 0;
		for(int j = 1 ; j <= this.nbNonBasicVar ; j++) {
			if(this.D[0][j] > maxCoeff) {
				maxCoeff = D[0][j];
				indexMaxCoeff = j;
			}
		}
		return indexMaxCoeff;
	}
	...
	public int searchEnteringAdvantageousVariableIndex() {	
		// Check if there is a candidate to enter the basic set
		// if there is not, return 0
		// in this case, we have found the final dictionary
		boolean lastDictionary = true;
		for(int i = 1 ; i <= this.nbNonBasicVar ; i++) {
			if (this.D[0][i] > 0) {
				lastDictionary = false;
				break;
			}
		}
		if(lastDictionary) {
		...
		// Check if problem is bounded by checking if there exists a candidate
		// to enter that has a candidate to leave (using previous function)
		boolean isBounded = false;
		double maxGain = 0.0;
		double gain;
		int enteringVarIndex = 0;
		int leavingCandidate;
		for(int j = 1 ; j <= this.nbNonBasicVar ; j++) {
			leavingCandidate = searchIndexLeavingVariable(j);
			if(leavingCandidate != 0) {
				isBounded = true;
				// Search for the entering variable that maximizes the 
				// objective function in the next dictionary
				gain = this.D[leavingCandidate][0] * this.D[0][j];
				if(gain > maxGain) {
					maxGain = gain;
					enteringVarIndex = j;
				}
	...
	public int searchIndexEnteringVariableSmallestNumber(){		
		...
		for(int j = 0 ; j <= this.nbNonBasicVar ; j++) {
			if(this.D[0][j] > 0 && this.arrayNonBasicVar[j] < smallestNumber) {
				enteringVarIndex = j;
				smallestNumber = this.arrayNonBasicVar[j];
			}
		}
		return enteringVarIndex;
	}
	...
	public int searchIndexLeavingVariableSmallestNumber(int jE) {		
		...
		for(int i = 1 ; i <= this.nbBasicVar ; i++) {
			if(this.D[i][jE] < 0) {
				isBounded = true;
				if(arrayBasicVar[i] < minVal) {
					minVal = arrayBasicVar[i];
					minIndex = i;
				}	
			}
		}
